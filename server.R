...
#### CUSUM_chart plot1 function#############################################################################################
  CUSUM_plot1 <- function(z,j,L,U,Main.title){
...
#### CUSUM_chart plot2 function ############################################################################################# 
  CUSUM_plot2 <- function(z,j,L,U,Main.title){
  ...
  output$RT_CUSUM <- renderPlotly({
     ...
     if(input$pep1.1 == "all peptides") {
       ...
       results <- lapply(c(1:nlevels(prodata$Precursor)), function(j){
         ...
         plots[[2*j-1]] <<- CUSUM_plot1(z,j,input$L,input$U,"Retention Time")
         plots[[2*j]] <<- CUSUM_plot2(z,j,input$L,input$U,"Retention Time")
       })
     ...
     z=scale(x[1:length(x)],mu,sd) # transformation for N(0,1) )
      ...
      plot1 <- CUSUM_plot1(z,j,input$L,input$U, "Retention Time")
      plot2 <- CUSUM_plot2(z,j,input$L,input$U, "Retention Time")
